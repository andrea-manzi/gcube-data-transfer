...
public class GridFTPUtil {
    ...
    /**
     *  @param path - absolute path or relative path from current direcotry
     *    of the directory you want to list.
     */
     ...
     * @param srcDir - absolute full path of directory you want to transfer on sourcehost
     * @param destDir - absolute full path of either an IMAGINARY directory you
     * want to create via the transfer or an existing directory on destination host.
     * Note, you can re-name the directory name, e.g. this is ok:
     *  srcDir =  "/home/dave/dir"
     *  destDir = "/home/djm76/dirCopied"
     */
    public void thirdPartyTransferDir2(GridFTPClient srcGFC, String srcDir,
        ...
        while (enumeration.hasMoreElements()) {
            ...
            } else if (finfo.isDirectory()) {  // directory !finfo.getName().startsWith("."))
    ...
    /**
     * Xiaobo Yang, Dave Meredith
     * 3rd party transfer file
     * if file > 1MB transfer in parallel mode, otherwise in non-parallel mode
     *
     * @param srcFullpathFile - absolute full path of the source file (must start with "/")
     * @param destFullpathFile - absolute full path of the dest file (must start with "/")
     *
     */
    public void thirdPartyFileTransfer(GridFTPClient srcGFC,
        String srcFullpathFile, GridFTPClient destGFC,
        String destFullpathFile, boolean b_append)
        throws Exception {
        ...
        int startIndex = srcFullpathFile.lastIndexOf(SEP);
        String srcFileName = srcFullpathFile.substring(startIndex + 1, srcFullpathFile.length());
        ...
        if (fileSize > 1048576) { // file > 1MB transfer in parallel mode
            ...
            {
                //do nothing
                System.err.println("Exception found while doing an extendedTransfer: " + e.getMessage());
            }
     ...
     * if file > 1MB transfer in parallel mode, otherwise in non-parallel mode
     *
     * @param srcDir - absolute full path of the source dir (must start with "/")
     * @param srcFile - the name of the file you want to transfer (just name, no path info)
     * @param destDir - absolute full path of the dest dir (must start with "/")
     * @param destFile - the name of the file on on the destination host
     *     (note, you can specify a new name for the file - just name, no path info)
     */
    public void thirdPartyFileTransfer(GridFTPClient srcGFC,
     ...
     * @param dir - absolute path or relative path from users home directory of
    ...
    /**
     * @param fileDirPath - absolute path or relative path from users home directory
     *  of the file or directory you want to check if exists.
     */
    ...
    /**
     * Delete a remote directory (can delete both empty and not-empty dirs)
     * Wrapper around deleteRemoteSubDir2.
     * @param absoluteDirPath - the absolute path of the directory you want to delete
     */
    public void deleteRemoteSubDir(GridFTPClient gridFTPClient,
        ...
        /*if(true) {
        System.out.println("Full Dir  :  "+absoluteDirPath);
        System.out.println("Parent Dir:  "+dir_parent);
        System.out.println("Delete Dir:  "+dir_name);
        return;
        }*/
    ...
    /**
     * Xiaobo Yang, David Meredith
     * Delete directory on remote host
     * @param dir_parent - absolute full path of the parent directory
     *  (i.e. the parent directory of dir_sub)
     * @param dir_sub - the name of the directory you want to delete in dir_parent
     *  (note, this is just the 'name' of the directory, no path information).
     */
    public void deleteRemoteSubDir2(GridFTPClient gridFTPClient,
        ...
        if (totalNumber == 0) { // dir on host is empty
        ...
        } else { // dir_sub is not empty - can contain hidden elems.
            ...
            while (enumeration.hasMoreElements()) {
                ...
                } else if (finfo.isDirectory()) { // sub-directory exists
                    i_subDir++;
  ...
  /*public Vector<FileAttributes> toFileAttributes(Vector v, String imageLocation) {
        ...
        while (enumeration.hasMoreElements()) {
            ...
            if (!finfo.isSoftLink()) {
                ...
                try {
                    fattr.setName(java.net.URLEncoder.encode(finfo.getName(), "UTF-8"));
                } catch (UnsupportedEncodingException ue) {
                    fattr.setName(finfo.getName());
                    //System.out.println("[GridFTPUtil] UnsupportedEncodingException caught - " + ue.toString());
                }
                ...
                if (finfo.isFile()) {
                    ...
                    if (ext.equals("ps"))
                        fattr.setPiclink(imageLocation + "ps.gif");
                    else if (ext.equalsIgnoreCase("pdf"))
                        fattr.setPiclink(imageLocation + "pdf.gif");
                    else if (ext.equalsIgnoreCase("zip"))
                        fattr.setPiclink(imageLocation + "zip.gif");
                    else if (ext.equalsIgnoreCase("jar"))
                        fattr.setPiclink(imageLocation + "zip.gif");
                    else if (ext.equalsIgnoreCase("gz"))
                        fattr.setPiclink(imageLocation + "gz.gif");
                    else if (ext.equalsIgnoreCase("jpg"))
                        fattr.setPiclink(imageLocation + "image.gif");
                    else if (ext.equalsIgnoreCase("gif"))
                        fattr.setPiclink(imageLocation + "image.gif");
                    else if (ext.equalsIgnoreCase("tif"))
                        fattr.setPiclink(imageLocation + "image.gif");
                    else if (ext.equalsIgnoreCase("mpg"))
                        fattr.setPiclink(imageLocation + "mpg.gif");
                    else if (ext.equalsIgnoreCase("mov"))
                        fattr.setPiclink(imageLocation + "mov.gif");
                    else if (ext.equalsIgnoreCase("htm"))
                        fattr.setPiclink(imageLocation + "web.gif");
                    else if (ext.equalsIgnoreCase("txt"))
                        fattr.setPiclink(imageLocation + "text.gif");
                    else if (ext.equalsIgnoreCase("java"))
                        fattr.setPiclink(imageLocation + "java.gif");
                    else if (ext.equalsIgnoreCase("c"))
                        fattr.setPiclink(imageLocation + "c.gif");
                    else if (ext.equalsIgnoreCase("cpp"))
                        fattr.setPiclink(imageLocation + "cpp.gif");
                    else if (ext.equalsIgnoreCase("hpp"))
                        fattr.setPiclink(imageLocation + "cpp.gif");
                    else if (ext.equalsIgnoreCase("cc"))
                        fattr.setPiclink(imageLocation + "cpp.gif");
                    else if (ext.equalsIgnoreCase("h"))
                        fattr.setPiclink(imageLocation + "h.gif");
                    else if (ext.equalsIgnoreCase("class"))
                        fattr.setPiclink(imageLocation + "class.gif");
                    else if (ext.equalsIgnoreCase("o"))
                        fattr.setPiclink(imageLocation + "obj.gif");
                        ...
                        fattr.setPiclink(imageLocation + "msword.gif");
                    else if (ext.equalsIgnoreCase("ppt"))
                        fattr.setPiclink(imageLocation + "ppt.gif");
                    else if (ext.equalsIgnoreCase("xls"))
                        fattr.setPiclink(imageLocation + "excel.gif");
                    else
                        fattr.setPiclink(imageLocation + "file.gif");
                    ...
                    fattr.setisFile("2");
                    // ends
                    fattr.setPiclink(imageLocation + "folder.gif");
                } else
                    fattr.setPiclink(imageLocation + "folder.gif");
        ...
        //System.out.println("[GridFTPUtil] toFileAttributes() finished");
        return dirV;
    }*/

/*
 * package uk.ac.dl.escience.vfs.util;
public class FileAttributes implements java.io.Serializable {
    private String name = "";
    private String time = "";
    private String size = "";
